...
invocation input to the child workflow. Prelock preserves the
...
Neither product defines Prelock semantics.
...
Prelock makes the declared execution happen and preserves enough
execution truth for the workflow to continue correctly afterward.

Prelock MUST NOT invent workflow strategy, business semantics, or
...
Prelock MUST NOT determine domain-specific progression on the
workflow's behalf.

For example, Prelock MUST NOT decide that:
...
workflow-owned decisions across runtime execution boundaries. Prelock
...
A new workflow MUST be able to use Prelock without requiring
Prelock to be modified merely because that workflow introduces new
domain semantics.
...
known, Prelock MUST NOT silently reinterpret that condition as:
...
Prelock does NOT thereby promise universal exactly-once external
effects.
...
Prelock must be able to execute or cause execution of such work and
...
Prelock does not currently require that every main-agent continuation
...
At minimum, Prelock must support shapes such as:
...
Prelock owns the generic execution facts required to resume control
correctly.
...
progressed by a main-agent controller, Prelock must prevent
...
* workflow-defined execution that is internally concurrent if such a capability
  is later admitted;
* concurrency outside Prelock's control.
...
Prelock MUST NOT impose global serialization merely because another
...
Prelock creates and executes the child `WorkflowExecution`, preserves
...
Prelock MUST NOT need to interpret the business meaning of the
...
Prelock MUST NOT be required to validate the business authority or
...
For example, Prelock may need to know conceptually:
...
complete, Prelock must preserve that terminal execution fact and return
the workflow's result to its immediate caller where one exists.

Prelock MUST NOT infer workflow completion merely because:
...
Prelock is an execution-continuity substrate, not a generic topology
execution system.

The governing workflow retains progression authority. Prelock realizes
...
workflow requests one execution
        ↓
Prelock realizes that execution
        ↓
...
Prelock does not need first-class knowledge that successive decisions
...
Prelock MUST NOT acquire first-class generic workflow-topology
...
Every first-class Prelock capability MUST be justified by an accepted
...
Prelock is deliberately allowed to remain incomplete as a general
workflow system.
...
For every proposed Prelock capability, invariant, or architecture
decision, ask:

> **Can an externally defined workflow preserve ownership of its own semantics
> and progression while Prelock provides only the reusable execution
> continuity, control-transfer, execution-occurrence, and workflow-call
> machinery required to realize those decisions?**

A design is suspect if Prelock must understand why a workflow requested
an action.
...
Prelock MUST NOT acquire first-class semantics for generic workflow
...
Prelock carries the input and makes it available to the child without
...
Semantic domain state owned by the workflow, whose meaning Prelock does
not need to understand.

## Runtime execution state

Generic execution facts owned by Prelock that are required to preserve
control and continuation.
...
One boundary invariant identity (`PRELOCK-INV-001`) has been explicitly
